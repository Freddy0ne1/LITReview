...
def photo_upload(request):
    """
    Vue pour uploader une photo

    Permet d'uploader une image avec une légende optionnelle
    L'image sera automatiquement redimensionnée à 800x800 pixels max
    """

    if request.method == "POST":
        # request.FILES est obligatoire pour l'upload de fichiers
        ...
        if form.is_valid():
            ...
            messages.success(request, "Photo publiée avec succès !")
...
def ticket_and_review_create(request):
    ...
    if request.method == "POST":
        ...
        if ticket_form.is_valid() and review_form.is_valid():
            try:
                # Transaction atomique : garantit que tout se passe ou rien
    ...
    return render(request, "blog/ticket_review_create.html", {
...
def edit_blog(request, blog_id):
    """
    Vue pour modifier ou supprimer un article de blog
    ...
    if blog.author != request.user:
        messages.error(request, "Vous n'êtes pas autorisé à modifier ce blog.")
        return redirect("feed")
    ...
    if request.method == "POST":
        ...
        # Action : Supprimer l'article
        elif "delete_blog" in request.POST:
            delete_form = forms.DeleteBlogForm(request.POST)
            if delete_form.is_valid():
                blog.delete()
                messages.success(request, "Votre blog a été supprimé.")
                return redirect("feed")
...
def user_posts(request):
    ...
    user_tickets = models.Ticket.objects.filter(user=request.user).order_by('-time_created')
    user_reviews = models.Review.objects.filter(user=request.user).order_by('-time_created')

    # Critiques reçues (écrites par d'autres utilisateurs)
    reviews_received = models.Review.objects.filter(
...
def review_create(request, ticket_id):
    ...
    if block_exists:
        messages.error(request, "Vous ne pouvez pas répondre à ce ticket en raison d'un blocage.")
        return redirect("feed")

    # Vérifier si l'utilisateur a déjà répondu
    existing_review = models.Review.objects.filter(ticket=ticket, user=request.user).first()
    if existing_review:
    ...
    return render(request, "blog/review_create.html", {
        "review_form": review_form,
        "ticket": ticket
    })
...
def delete_review(request, review_id):
    ...
    messages.success(request, f"La critique '{review_headline}' a été supprimée avec succès.")
    return redirect("user_posts")