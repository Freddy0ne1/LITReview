...
class User(AbstractUser):
    ...
    def get_following(self):
        ...
        return User.objects.filter(
            id__in=self.following.values_list('followed_user_id', flat=True)
        )

    def get_followers(self):
        """Retourne la liste des abonnés"""
        return User.objects.filter(
            id__in=self.followed_by.values_list('user_id', flat=True)
        )

    def is_following(self, user):
        """Vérifie si cet utilisateur en suit un autre"""
        return self.following.filter(followed_user=user).exists()
...
class UserFollows(models.Model):
    ...
    Relations inversées :
    - user.following.all() → Abonnements où user est le follower
    - user.followed_by.all() → Abonnements où user est suivi
    """
